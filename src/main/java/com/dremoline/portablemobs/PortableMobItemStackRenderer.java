    ...
    public void renderByItem(ItemStack itemStack, ItemCameraTransforms.TransformType transformType, MatrixStack matrixStack, IRenderTypeBuffer typeBuffer, int combinedLight, int combinedOverlay) {
        ...
        if (optional.isPresent()) {
            ...
            if (living != null) {
                ...
                float size = 0.9f;
                float length = size / (float) Math.sqrt(2 * width * width + height * height);
                matrixStack.scale(length, length, length);

                matrixStack.translate(0.5, 0.5, 0.5);
                matrixStack.mulPose(new Quaternion(0, (System.currentTimeMillis() % 1000) / 1000f * 360, 0, true));
                System.out.println("rotation: "+((System.currentTimeMillis() %1000) /1000f * 360));
